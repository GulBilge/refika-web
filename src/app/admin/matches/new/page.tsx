...
export default function MatchingNewPage() {
  ...
  const [matching_pairs, setMatchingPairs] = useState<MatchingPair[]>([
    // İlk kelime çiftini otomatik ekliyoruz, böylece kullanıcı hemen veri girmeye başlayabilir.
    { id: uuidv4(), left_text: '', right_text: '' }
  ])
  const [terms, setTerms] = useState<Term[]>([])
  const [loading, setLoading] = useState(true)
  const [saving, setSaving] = useState(false)

  // Dönem verilerini yükleme işlemi
  useEffect(() => {
    const loadTerms = async () => {
      ...
      try {
        ...
        toast.error('Dönem verileri yüklenirken hata oluştu: ' + e.message)
      } finally {
        setLoading(false)
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      // 1. Veri Doğrulama
      if (!title || !title.trim() ) {
        toast.error('Başlık seçimi zorunludur.')
        setSaving(false)
        return
      }
      if ( !termId) {
      ...
      if (matching_pairs.length === 0) {
        toast.error('En az bir eşleştirme çifti eklemelisiniz.')
        setSaving(false)
        return
      }
      ...
      // 2. Yeni Eşleştirme Oyununu Kaydet
      const { data: newMatchingGame, error: gameError } = await supabase
        .from('activities')
        .insert({
          title,
          term_id: termId,
          activity_type: 'matching', // Yeni eşleştirme oyunu için type ekleniyor
        })
        .select('id')
        .single()
      ...
      const newMatchingId = newMatchingGame.id

      // 3. Eşleştirme Çiftlerini Kaydet
      const pairsToInsert = matching_pairs.map(p => ({
        game_id: newMatchingId,
      ...
      toast.success('Yeni Eşleştirme Oyunu başarıyla oluşturuldu! 🎉')
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-6 sm:p-8">
        ...
        <div className="flex items-center mb-6">
          ...
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800">Yeni Eşleştirme Oyunu Oluştur</h1>
        </div>
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 shadow-top">
          <div className="max-w-4xl mx-auto flex justify-end">
            <button
              ...
              className="w-full sm:w-auto bg-green-600 text-white font-bold px-8 py-3 rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors shadow-md"
            >
